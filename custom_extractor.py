...
def extract_features(data_dir, output_csv, log_file, custom_mode=False):
    class_mapping = {
    ...
    if custom_mode:
        for img_name in image_files:
            ...
            base_class = class_mapping.get(img_name)
            if base_class is None:
                logging.warning(f"No mapping found for: {img_name}. Skipping.")
                print(f"Skipping image (no mapping): {img_name}")
                continue
        ...
        # === Original training/test dataset directory structure ===
        for fruit_class in os.listdir(data_dir):
            class_path = os.path.join(data_dir, fruit_class)
            if not os.path.isdir(class_path):
                continue

            base_class = class_mapping.get(fruit_class)
            if base_class is None:
